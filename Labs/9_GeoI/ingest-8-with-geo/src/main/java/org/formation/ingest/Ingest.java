...
public class Ingest {
	...
	public static void main(String[] args) throws IOException {
		...
		double parisLat=48.856614f;
		double parisLong=2.3522219f;
		...
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir,
			...
			for (Path file : stream) {
				...
				double latitude = parisLat  + (random.nextFloat() - 0.5)*5;
				double longitude = parisLong  + (random.nextFloat() - 0.5)*5;
				Map<String, Object> jsonMap = new HashMap<>();
				jsonMap.put("name", file.getFileName());
				jsonMap.put("data", encodedString);
				jsonMap.put("location",  latitude +","+longitude);
